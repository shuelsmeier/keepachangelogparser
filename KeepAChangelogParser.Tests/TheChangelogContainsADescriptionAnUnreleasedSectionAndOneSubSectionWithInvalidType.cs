...
{
  [TestClass]
  public class TheChangelogContainsADescriptionAnUnreleasedSectionAndOneSubSectionWithInvalidType
  {
    [TestMethod]
    public void WhenTheChangelogContainsADescriptionAnUnreleasedSectionAndOneSubSectionWithInvalidType_ThenTheResultIsFailureInvalidSubsectionType()
    {
      // Arrange
      string textToParse =
        createTextToParse();
    ...
    private static Result<Changelog> createExpectedParseResult()
    {
      Result<Changelog> expectedParseResult =
        Result.Failure<Changelog>(
          "Invalid subsection type. Error parsing text in line 16 / index 5.");
