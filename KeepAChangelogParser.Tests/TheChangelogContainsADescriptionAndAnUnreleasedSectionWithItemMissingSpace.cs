...
namespace KeepAChangelogParser.Tests
{
  ...
  [TestClass]
  public class TheChangelogContainsADescriptionAndAnUnreleasedSectionWithItemMissingSpace
  {

    [TestMethod]
    public void WhenTheChangelogContainsADescriptionAndAnUnreleasedSectionWithItemMissingSpace_ThenTheResultIsFailureNoSpace()
    {
      // Arrange
      string textToParse =
        createTextToParse();
    ...
    private static Result<Changelog> createExpectedParseResult()
    {
      Result<Changelog> expectedParseResult =
        Result.Failure<Changelog>(
          "No space. Error parsing text in line 12 / index 2.");
