...
{
  [TestClass]
  public class TheChangelogContainsADescriptionAndAnUnreleasedSectionWithInvalidType
  {
    [TestMethod]
    public void WhenTheChangelogContainsADescriptionAndAnUnreleasedSectionWithInvalidType_ThenTheResultIsFailureInvalidSubsectionType()
    {
      // Arrange
      string textToParse =
        createTextToParse();
    ...
    private static Result<Changelog> createExpectedParseResult()
    {
      Result<Changelog> expectedParseResult =
        Result.Failure<Changelog>(
          "Invalid subsection type. Error parsing text in line 10 / index 5.");
